<script lang="ts">
    ...
    let { scale, showAnomalies, showOcr, ocrColor, suspectedFraud, onZoomIn, onZoomOut, onReset, onToggleAnomalies, onToggleOcr, onOcrColorChange, onSuspectedFraudChange }: Props = $props();

    const scalePercent = $derived(Math.round(scale * 100));
    const currentColor = $derived(ocrColors.find((c: OcrColor) => c.key === ocrColor) ?? ocrColors[0]);
</script>

<div class="flex items-center gap-3 rounded-lg border border-zinc-200 bg-white p-1 pl-4">
    <Toggle label="Anomalies"
    ...
    <Popover.Root>
        <Popover.Trigger
            class="size-6 rounded-full {currentColor.swatch} outline-none ring-offset-2 transition-transform hover:scale-110 focus:ring-2 focus:ring-yellow-400"
